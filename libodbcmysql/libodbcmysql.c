...
typedef struct {
  ...
  #include "libodbcmysql_config.def"
  #undef CFG
} libodbcmysql_config_t;
...
static bool _libodbcmysql_is_initialized = false;
static void * _libodbcmysql_dlhandle = NULL;
static libodbcmysql_config_t _libodbcmysql_configuration = {
...
#define DEFINE_MYSQL_FPTR(name)                                         \
  libodbcmysql_##name##_fptr_t libodbcmysql_dlsym_##name = NULL;
...
static int
_libodbcmysql_inih_cb (
  void                 *user,
  const char           *section,
  const char           *name,
  const char           *value
) {
  libodbcmysql_config_t *cfg = (libodbcmysql_config_t *) user;
...
void
_libodbcmysql_load_configuration (
  void
) {
  char *ini_file_path = getenv("LIBODBCMYSQL_INI");

  if (NULL != ini_file_path) {
    if (0 > ini_parse(ini_file_path, _libodbcmysql_inih_cb,
      &_libodbcmysql_configuration)) {
...
#ifndef WIN32
      "~/.libodbcmysql.ini",
      "/etc/libodbcmysql.ini"
#endif /* UNIX */
    };
    for (int ii = 0; ii < (sizeof(search_path) / sizeof(char *)); ++ii) {
      printf("parsing %s\n", search_path[ii]);
      int rc = ini_parse(search_path[ii], _libodbcmysql_inih_cb,
        &_libodbcmysql_configuration);
      printf("rc = %d\n", rc);
      if (0 == rc) {
  ...
  int log_level = (int) strtol(_libodbcmysql_configuration.default_log_level,
    (char **) NULL, 10);
  switch (log_level) {
    case LOG_TRACE:
    case LOG_DEBUG:
    ...
    default:
      log_error("Invalid log level (%s)",
        _libodbcmysql_configuration.default_log_level);
      log_set_level(LOG_ERROR);
  }

  /* Dump the config we've read */
  dump_config(&_libodbcmysql_configuration);

  /* Load dynamic */
  if (NULL != _libodbcmysql_configuration.default_dlopen) {
    _libodbcmysql_dlhandle = dlopen(_libodbcmysql_configuration.default_dlopen,
      (RTLD_LOCAL | RTLD_LAZY));
    if (NULL != _libodbcmysql_dlhandle) {
      #define ASSIGN_MYSQL_FPTR(name)                                         \
        do {                                                                  \
          if (NULL != _libodbcmysql_configuration.name##_dlsym) {             \
            MYSQL_FPTR(name) = MYSQL_FPTR_TYPE(name)                          \
              dlsym(_libodbcmysql_dlhandle, #name);                           \
...
libodbcmysql_init (
  ...
  if (true == _libodbcmysql_is_initialized) {
    return;
  }
  ...
  /* One round only */
  _libodbcmysql_is_initialized = true;
